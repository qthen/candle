...
'''
The success of PCA seems to suggest that the problem is very linear in structure.
Here I present a linear model and evaluate its usefulness

Preliminary results suggest it is less powerful than the nonlinear fully connected models, about 5 percent worse
'''

class LinearNN(object):

	# Constant for where to save this model
	MODEL_FILEPATH = "models/regression/BaselineNN.h5"

	'''
	Creates the linear neural network with separate fully connected layers on stellar spectra
	Inputs:
	...
	def __init__(self, S_D = 10, shared = False):
		...
		if shared:
			# Shared dense layers
			dense_layer_1 = Dropout(0.1)(Dense(128, activation='linear')(input_spectra))
			dense_layer_2 = Dropout(0.1)(Dense(64, activation='linear')(dense_layer_1))
