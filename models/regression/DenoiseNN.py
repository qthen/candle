...
'''
A very noisy neural model that randomly turns off all input pixels with probability p
This is mainly to test at how important each pixel value is relative (I would think that most of the important information is concentrated in very few pixels)
Should take in entire spectra
'''

class DenoiseNN(object):

	# Constant for where to save this model
	MODEL_FILEPATH = "models/regression/BaselineNN.h5"

	'''
	Creates the denoise neural network
	Inputs:
